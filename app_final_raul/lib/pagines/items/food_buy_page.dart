...
class BuyFoodPage extends StatefulWidget {
  @override
  _BuyFoodPageState createState() => _BuyFoodPageState();
}

class _BuyFoodPageState extends State<BuyFoodPage> {
  ...
  double _getPriceBySize(String itemName) {
    if (itemName.contains("HAM")) return 10.0;
    if (itemName.contains("BAll")) return 5.0;
    return 3;
  }

  double _calculateItemTotal(Item item) {
    double price = _getPriceBySize(item.nom);
    return (productQuantities[item.id] ?? 0) * price;
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<Item> waterItems =
        itemsProvider.items.where((item) => item.tipus == "comida").toList();
    final Usuari? usuari =
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Comprar Comidas'),
